...
public class Transitions {
	...
	public Transition getTransitionByElements(long sourceStateId, long targetStateId, long eventId) {
		Transition t;
	...
	public Vector<Transition> getTransitionsBySourceId_and_TargetId(long sourceStateId, long targetStateId) {
		Transition t;
		...
		for (int i = 0; i < size(); i++) {
			...
			if ((t.idStateSource == sourceStateId) && (t.idStateTarget == targetStateId)) {
				ts.add(t);
			}
	...
	public Transition exists(Transition t) {
		...
		for (int i = 0; i < size(); i++) {
			...
			if ((tmp.idStateSource == t.getIdStateSource()) && (tmp.idStateTarget == t.getIdStateTarget())) {
				if (utils.equalContent(tmp.getTransitionContent(), t.getTransitionContent()))
					return tmp;
			}
		}
		return null;
	...
	public int getLowerTransitionExecIndex() {
		...
		for (int i = 0; i < size(); i++) {
			t = transitions.get(i);
			if ((t.getTransitionExecIndex() < lower) && (t.getTransitionExecIndex() >= 0))
