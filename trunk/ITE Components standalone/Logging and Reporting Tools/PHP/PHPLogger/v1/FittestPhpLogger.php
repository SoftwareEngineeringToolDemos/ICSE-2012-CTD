...
class FittestPhpLogger
{
	...
	static $php_data_start; /* by urueda: manages data start */
	static $php_data_end; /* by urueda: manages data end */
	
	static function startLog()
	{
		self::$php_data_start = FALSE;
		self::$php_data_end = FALSE;
	...
	{
		self::$logfile_handle->endElement("phplog");
	...
	{ 
        ...
        self::output_callback("", PHP_OUTPUT_HANDLER_END); /* by urueda: force php output end */
	...
	{
		/* begin by urueda */
		if (self::$php_data_start && ! self::$php_data_end)
		{
			self::$logfile_handle->endElement("data");
			self::$php_data_end = TRUE;	
		} /* end by urueda */
	...
	{ 	
		...
		if($flag & PHP_OUTPUT_HANDLER_START)
		{
			...
			self::$php_data_start = TRUE; /* by urueda */
		}
		...
		{
			self::$logfile_handle->endElement();
			self::$php_data_end = TRUE; /* by urueda */
		}	
		return FALSE;
	}
