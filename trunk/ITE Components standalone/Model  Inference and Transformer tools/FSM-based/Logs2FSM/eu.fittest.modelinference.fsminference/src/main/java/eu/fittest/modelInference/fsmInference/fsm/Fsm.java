...
public class Fsm {
	...
	Config_Launcher config = Config_Launcher.getInstace();
	...
	public Vector<Long> getKeyOfEdgeAndExecIndex_ByValue(int value) {
		...
		for (int i = 0; i < transitions.size(); i++) {
			if (transitions.getTransitions().get(i).getTransitionExecIndex() == value) {
				tobedeleted.add(transitions.getTransitions().get(i)
						.getidTransition());
			}
		}
	...
	public boolean addItemToStates(State stateBefore, Transition event,
			State stateAfter, int execIndex) {

		boolean added = addItemToStates(stateBefore.getStateContent(),
				event.getTransitionContent()[0], stateAfter.getStateContent(),
				execIndex);
	...
	public boolean addItemToStates(String[] stateBefore, String event,
			String[] stateAfter, int execIndex) {
		String[] events = null;
		...
		if (tid == -1) {
			...
			if (transitions.getTransitionById(tid) != null)
				transitions.getTransitionById(tid).setTransitionExecIndex(
						execIndex);
			added = false;
		}
	...
	public long contains_TransitionId(String[] stateBefore, String event,
			String[] stateAfter) {
		...
		if (idT > -1) {
			...
			if ((idSource > -1) && (idTarget > -1)) {
				if (utils
						.equalContent(
								states.getStateById(idSource).stateContent,
								stateBefore)) {
					if (utils.equalContent(
							states.getStateById(idTarget).stateContent,
							stateAfter)) {
						return idT;
					}
				}
	...
	public void cleaningFsm() {
		...
		for (int i = 0; i < states.size(); i++) {
			...
			while ((j < transitions.size()) && (isused == false)) {
				if ((transitions.getTransitions().get(j).idStateSource == idStateTocheck)
						|| (transitions.getTransitions().get(j).idStateTarget == idStateTocheck)) {
					isused = true;
				}
				j++;
			}
	...
	public Vector<Vector<String[]>> convertFormat() {
		...
		for (int i = 0; i < transitions.size(); i++) {
			...
			s = states
					.getStateById(transitions.getTransitions().get(i).idStateSource);
			if (s != null) {
				contentBefore = utils.copy(s.getStateContent());

				s = states
						.getStateById(transitions.getTransitions().get(i).idStateTarget);
				if (s != null) {
					contentAfter = utils.copy(s.getStateContent());

					tran.add(contentBefore);
					tran.add(utils.copy(transitions.getTransitions().get(i)
	...
	public Fsm clone() {
		...
		for (int i = 0; i < this.transitions.getTransitions().size(); i++) {
			...
			f.addItemToStates(source.getStateContent(),
					t.getTransitionContent()[0], target.getStateContent(),
					execIndex);
		}
	...
	public double eventDensity(String event) {
		...
		for (int i = 0; i < transitions.size(); i++) {
			if (transitions.getTransitions().get(i).getTransitionContent()[0]
